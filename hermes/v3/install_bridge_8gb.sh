mkdir -p /root/claude-knowledge/playbooks /root/backups /root/pipelines

cat > /root/claude-knowledge/bridge_server.py << 'CLAUDEEOF'
#!/usr/bin/env python3
"""
...
TOKEN = os.environ.get("HERMES_BRIDGE_TOKEN", "synapt8gb_cW61smNnlWHxlnseiZqgCOhmW-r3W-GvuD0fzal3x90")
...
def cmd_tail_log(params):
    ...
    ALLOWED_PREFIXES = ["/var/log/", "/root/claude-knowledge/", "/root/backups/"]
    path = params.get("path", "")
...
def cmd_run_playbook(params):
    # Playbooks: scripts predefinidos en /root/claude-knowledge/playbooks/<name>.sh
    # NUNCA ejecuta scripts arbitrarios — solo los que ya existen en ese directorio.
    name = params.get("name", "")
    if not re.fullmatch(r"[a-zA-Z0-9_\-]+", name or ""):
        raise ValueError("nombre de playbook inválido")
    path = f"/root/claude-knowledge/playbooks/{name}.sh"
...
def cmd_write_file(params):
    ...
    ALLOWED_PREFIXES = ["/root/claude-knowledge/", "/root/pipelines/"]
    path = params.get("path", "")
...
cat > /etc/systemd/system/claude-bridge.service << 'UNIT'
[Unit]
Description=Claude Bridge Server (synapt-8gb)
After=network.target

[Service]
ExecStart=/usr/bin/python3 /root/claude-knowledge/bridge_server.py
WorkingDirectory=/root/claude-knowledge
Restart=always
User=root
...
sleep 1
systemctl status claude-bridge --no-pager | head -6
echo ""
